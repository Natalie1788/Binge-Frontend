        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-75 z-50" onClick={onClose}>
            <div className="bg-white p-4 md:p-6 rounded-lg shadow-lg w-full max-w-lg mx-4 sm:mx-auto overflow-y-auto max-h-full " onClick={(e) => e.stopPropagation()}>
                <div className="flex justify-between items-center ">
                    <h2 className="text-xl md:text-2xl font-bold text-center md:text-left flex-1">{dish.dishName}</h2>
                ...
                <h3 className="mt-6 text-lg font-semibold md:text-left sm:text-center">Ingredients</h3>
                <ul className="list-disc list-inside text-gray-700 mt-2">
                    {dish.ingredients.map((ingredient, index) => (
                        <li key={index}>{ingredient}</li>
                    ))}
                </ul>
                <h3 className="mt-6 text-lg font-semibold md:text-left sm:text-center">Recipe</h3>
                <p className="text-gray-700 mt-2 md:text-left sm:text-center">{dish.recipe}</p>
                <div className="mt-6">
